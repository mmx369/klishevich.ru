...
goodsRouter.get("/", async (request, response) => {
  ...
  const goodsPaperMoney = goods.filter((el) => el.amountOfGoods > 0 && String(el.category) === '601c638753bec224ffda838f')
  ...
  const goodsCoins = goods.filter((el) => el.amountOfGoods > 0 && String(el.category) === '601f072c36b3304ab510d160')
  let coinCategory;
  if (goodsCoins) {
  ...
  response.json({ goodsList: goods.map((u) => u.toJSON()), categoryList: paperMoneyCategory, coinList: coinCategory });
});
...
goodsRouter.post("/",
  ...
  async (request, response) => {
    ...
    try {
      ...
      if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array(), message: 'You entered wrong data' })
      }

      const { nameOfGoods, amountOfGoods, priceOfGoods, imagePath, category, country } = request.body
